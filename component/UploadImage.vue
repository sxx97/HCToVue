...
( () => {
            this.$vc.on('uploadImage', 'openAddApplicationKeyModal',  () => {

            });
            this.$vc.on('uploadImage', 'clearImage',  () => {
                this.uploadImageInfo = {
                    photos: []
                }
            });

            this.$vc.on('uploadImage', 'notifyPhotos',  (_photos) => {
                this.uploadImageInfo = {
                    photos: []
                };
                // 传入图片内容是地址链接的组件
                ...
                _photos.forEach( (_photo) => {
                    ...
                    this.$vc.urlToBase64(photoUrl + "?fileId=" + _photo + "&communityId=" + this.$vc.getCurrentCommunity().communityId + "&time=" + new Date(),  (_base64Data) => {
