
        <template>
            <div class="row">
    <div class="col-lg-12">
        <div class="ibox ">
            <div class="ibox-title">
                <h5>发布广告信息</h5>
                <div class="ibox-tools" style="top:10px;">
                    <button type="button" v-if="viewAdvertInfo.index != 2" class="btn btn-primary btn-sm"
                            style="margin-right:10px;" v-on:click="_openSelectAdvertInfoModel()">
                        <i class="glyphicon glyphicon-search"></i> 选择发布广告
                    </button>

                    <button type="button" v-if="viewAdvertInfo.index != 2" class="btn btn-primary btn-sm"
                            v-on:click="_openAddAdvertInfoModel()">
                        <i class="glyphicon glyphicon-plus"></i> 添加发布广告
                    </button>
                </div>
            </div>
            <div class="ibox-content">
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label">广告名称：</label>
                            <label class="">{{viewAdvertInfo.adName}}</label>
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label">广告类型：</label>
                            <label class="">{{viewAdvertInfo.adTypeCd}}</label>
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label">广告分类：</label>
                            <label class="">{{viewAdvertInfo.classify}}</label>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label">投放位置：</label>
                            <label class="">{{viewAdvertInfo.locationTypeCd}}</label>
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label">具体位置：</label>
                            <label class="">{{viewAdvertInfo.locationObjId}}</label>
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label">广告状态：</label>
                            <label class="">{{viewAdvertInfo.state}}</label>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label">播放顺序：</label>
                            <label class="">{{viewAdvertInfo.seq}}</label>
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label">投放时间：</label>
                            <label class="">{{viewAdvertInfo.startTime}}</label>
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label">结束时间：</label>
                            <label class="">{{viewAdvertInfo.endTime}}</label>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>

    <vc:create name="addAdvert"
               callBackListener="viewAdvertInfo"
               callBackFunction="chooseAdvert"
    ></vc:create>


    <vc:create name="chooseAdvert"
               emitChooseAdvert="viewAdvertInfo"
               emitLoadData="viewAdvertInfo"
    ></vc:create>
</div>

        </template>
        <script>
            
            import AddAdvert from '@/component/AddAdvert.vue';import ChooseAdvert from '@/component/ChooseAdvert.vue';



            const OWNER_TYPE = {
                OWNER: '1001',
                TENANT: '1003'
            };
            // 考核类型
            const ASSESSMENT_TYPE = {
                BUSINESS: 2,
                COMMON: 1,
                SYSTEM: 3,
            };
            
            // 删除时的类型
            const DELETE_TYPE = {
                TABLE: 1, // 删除通用考核表
                BUSINESS_ITEM: 2,  // 删除业务考核项
                COMMON_ITEM: 3, // 删除通用考核项
            }
            // 考核人员
            const ASSESSMENT_OBJECT_TYPE = {
                STAFF: 1,
                SUPERVISOR: 2,
                MANAGER: 3,
            }
            
            const AUTO_ASSESSMENT_TYPE = {
                INSPECTION: 1,
                WORK_ORDER: 2,
                ATTENDANCE: 3,
            }
            
            // 考核项目
            const ASSESSMENT_ITEM_TYPE = {
                COMMON_ITEM: 1,
                BUSINESS_ITEM: 2,
            }
            
            const DEFAULT_PAGE = 1;
            const DEFAULT_ROWS = 10;

            
            export default {
                props: {"callBackListener":"string","callBackFunction":"string"},
                components: {
                    AddAdvert,ChooseAdvert
                },
                data () {
                    return {"viewAdvertInfo":{"index":0,"flowComponent":"viewAdvertInfo","adName":"","adTypeCd":"","classify":"","locationTypeCd":"","locationObjId":"","state":"","seq":"","startTime":"","endTime":""}}    
                },
                mounted() {
                ( () => {
            //根据请求参数查询 查询 业主信息
            this._loadAdvertInfoData();
        })()
( () => {
            this.$vc.on('viewAdvertInfo', 'chooseAdvert',  (_app) => {
                this.$vc.copyObject(_app, this.viewAdvertInfo);
                this.$vc.emit(this.$vc._thiscallBackListener, this.$vc._thiscallBackFunction, this.viewAdvertInfo);
            });

            this.$vc.on('viewAdvertInfo', 'onIndex',  (_index) => {
                this.viewAdvertInfo.index = _index;
            });

        })()   
                },
                methods: {
                    _openSelectAdvertInfoModel() {
                this.$vc.emit('chooseAdvert', 'openChooseAdvertModel', {});
            },_openAddAdvertInfoModel() {
                this.$vc.emit('addAdvert', 'openAddAdvertModal', {});
            },_loadAdvertInfoData:function () {

            },
                },
            }
    
        </script>
        <style>
            
        </style>
    