    ...
    <div class="modal-dialog modal-lg">
        ...
        <script>
            ...
            export default {
                props: {"emitChooseService":"true","emitLoadData":"true"},
                components: {
                    
                },
                data () {
                    return {"chooseServiceInfo":{"services":[],"_currentServiceName":""}}    
                },
                mounted() {
                (() =>{
        })()
(() =>{
            this.$vc.on('chooseService','openChooseServiceModel',(_param) =>{
                console.log("打开业主成员界面")
                $('#chooseServiceModel').modal('show');
                this._refreshChooseServiceInfo();
                this._loadAllServiceInfo(1,10,'');
            });
        })()   
                },
                methods: {
                ...
                //发送get请求
               this.$vc.http.get('chooseService',
                            'list',
                             param,
                             (json) =>{
                                var _serviceInfo = JSON.parse(json);
                                this.chooseServiceInfo.services = _serviceInfo.services;
                             },() =>{
                                console.log('请求失败处理');
                             }
                           );
            },chooseService:function(_service){
                this.$vc.emit(this.$vc._thisemitChooseService,'chooseService',_service);
                this.$vc.emit(this.$vc._thisemitLoadData,'listServiceData',{
                    serviceId:_service.serviceId
                });
                $('#chooseServiceModel').modal('hide');
            },queryServices:function(){
                this._loadAllServiceInfo(1,10,this.chooseServiceInfo._currentServiceName);
            },_refreshChooseServiceInfo:function(){
                this.chooseServiceInfo._currentServiceName = "";
            },
                },
            }
