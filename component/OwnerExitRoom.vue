    ...
    <div class="modal-dialog" role="document">
        ...
        <script>
            ...
            export default {
                ...
                (undefined)()
(() =>{
             this.$vc.on('ownerExitRoom','openExitRoomModel',(_roomInfo) =>{
                    this.exitRoomInfo = _roomInfo;
                ...
                this.exitRoomInfo.communityId = this.$vc.getCurrentCommunity().communityId;
                this.$vc.http.post(
                    'ownerExitRoom',
                    'exit',
                    JSON.stringify(this.exitRoomInfo),
                    {
                        emulateJSON:true
                     },
                     (json,res) =>{
                        ...
                        if(res.status == 200){
                            //关闭model
                            $('#exitRoomModel').modal('hide');
                            this.$vc.emit('showOwnerRoom','notify',this.exitRoomInfo);
                            return ;
                        }
